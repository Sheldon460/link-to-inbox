...
> 每天都在 5 个 App 之间跳跃，9 个抓取工具换来换去？
> 我做了一个 WorkBuddy Skill，把所有外部链接统一归档到一个目录。
> **一个入口、一个格式、一个 Obsidian 收件箱。**
...
- B 站收藏了一个技术教程，需要的是官方字幕，不是 ASR 转写
...
**问题是：这些工具之间不互通。** 今天抓的文案在 A 工具的笔记里，明天抓的视频在 B 工具的下载目录里。下次想找的时候，我要在五个工具之间来回翻。
...
- **zsxq-cli**：知识星球的官方 CLI，OAuth 登录后能直接拿到 JSON，比网页抓取稳定得多
...
> 图 1：dousnap 一个站点就能处理 7 个视频/图文平台，粘链接 → 点提取 → 一次性拿到标题+描述+结构化文案+封面+视频下载链接。
...
![图 2：知识星球主题归档后 — frontmatter + 正文 + 元信息](images/fig-2-zsxq-markdown.svg)

> 图 2：知识星球主题归档后的 Obsidian 渲染效果。除了 YAML frontmatter，还保留了主题元信息（topic_id / group_id / 互动数据），方便后续按类型聚合。
...
![图 3：所有外部链接集中到 Obsidian 一个目录树下](images/fig-3-obsidian-tree.svg)

> 图 3：9 个平台归档目录统一命名规则：`MM.平台名/YYYY-MM-DD-作者-标题/`，所有外部链接集中到一个 Obsidian 收件箱。
...
**👉 仓库地址：https://github.com/Sheldon460/link-to-inbox**

**如果这篇文章对你有帮助，欢迎点赞 + 在看 + 转发。你的支持是我继续做的最大动力。**

你目前用的是什么抓取工具？遇到了什么坑？评论区聊聊。